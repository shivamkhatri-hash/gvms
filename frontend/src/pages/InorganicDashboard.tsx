...
export const InorganicDashboard: React.FC = () => {
  const [selectedDatasetId, setSelectedDatasetId] = useState<number | null>(null);
  const [searchParams] = useSearchParams();
  const datasetParam = searchParams.get('dataset') || 'trace_metal_';
  ...
  const igcDataset = datasets?.find((d) => d.name === datasetParam);

  useEffect(() => {
    if (igcDataset) {
      setSelectedDatasetId(igcDataset.id);
    }
  }, [igcDataset]);

  // Set default axes when variables load
  useEffect(() => {
    if (igcDataset?.variables) {
      const numericVars = igcDataset.variables.filter((v) => v.is_numeric);
  ...
  const { data: recordsData, isLoading: recordsLoading, refetch: refetchRecords } = useQuery({
    queryKey: ['dashboard-records-dynamic', selectedDatasetId, serializedFilters, page, pageSize, sortBy, sortDesc],
    queryFn: () =>
      api.get<{
        total: number;
        items: any[];
  ...
  const { data: chartData, isLoading: chartLoading } = useQuery({
    queryKey: ['custom-chart-data', selectedDatasetId, serializedFilters, xVar, yVar, colorBy, chartType],
    queryFn: () =>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-2">
        <div className="flex items-center gap-2">
          <Badge
            label={igcDataset?.display_name || 'metal lab'}
            variant="custom"
            customColor="bg-sky-100 text-sky-855 border-sky-200"
            size="md"
          />
          <span className="text-xs text-slate-400 font-semibold font-mono">
            {igcDataset?.sql_table_name}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <Button
              ...
              <div className="space-y-4">
                ...
                {metadata?.filter_options &&
                  Object.entries(metadata.filter_options)
                    .filter(([key]) =>
                      ['formation', 'location', 'name', 'ubhi', 'material_type'].includes(key)
                    )
                    .map(([key, opts]) => {
